...
function AddGrocery() {
 ...
 const handleSubmit = async (e: FormEvent) => {
  ...
  try {
    ...
    console.log("Added grocery:", result.data);
    setLoading(false);
  } catch (error: any) {
  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-linear-to-br from-green-50 to-white py-16 px-4 relative'>
      ...
      >
        ...
        <PlusCircle className='textg-green-600 w-8 h-8'/>
        ...
        <form className='flex flex-col gap-6 w-full' onSubmit={handleSubmit}>
          <div>
            ...
            onChange={(e)=>setName(e.target.value)} value={name} className='w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-green-400 transition-all'/>
          </div>
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
            <div>
              <label className='block text-gray-700 font-medium mb-1'>Category<span>*</span></label>
          ...
          <div>
              <label className='block text-gray-700 font-medium mb-1'>Unit<span>*</span></label>
              <select name='category' className='w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-green-400 transition-all bg-white' onChange={(e)=>setUnit(e.target.value)} value={unit}>
            ...
            onChange={(e)=>setPrice(e.target.value)} value={price} className='w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-green-400 transition-all'/>
          </div>
             <div className='felx flex-col sm:flex-row items-center gap-5'>
              <label htmlFor='image' className='cursor-pointer flex items-center justify-center gap-2 bg-green-50 text-green-700 font-semibold border border-green-200 rounded-xl px-6 py-3 hover:bg-green-100 transition-all w-full sm:w-auto'> <Upload className='w-5 h-5'/>Upload image 
              </label>
