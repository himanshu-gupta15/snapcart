...
import { ArrowLeft, Building, CreditCard, CreditCardIcon, Home, Loader2, LocateFixed, MapPin, Navigation, Phone, Truck, User } from 'lucide-react'
...
function Checkout() {
  ...
  useEffect(()=>{
    if(navigator.geolocation){
      ...
      },(err)=>{console.log('location error ',err)},{enableHighAccuracy: false, // VERY IMPORTANT
      timeout: 20000,            // give enough time
      maximumAge: 60000,})
    }
  },[]) 
...
,
  ...
  const handleOnelinePayment=async()=>{
  ...
  const handleCurrentLoacaiton=()=>{
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition((pos)=>{
        const {latitude,longitude}=pos.coords 
        setPosition([latitude,longitude])
      },(err)=>{console.log('loaction error',err)},{enableHighAccuracy: false, // VERY IMPORTANT
      timeout: 20000,            // give enough time
      maximumAge: 60000,})
    }
  }
  ...
  return  (
    <div className='w-[92%] md:w-[80%] mx-auto py-10 relative'>
      ...
      className='bg-white text-green-600 items-center justify-center rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 border border-gray-100 mb-1 mt-1 font-semibold  '
      >Checkout</motion.h1>
        ...
        >
          ...
          <div className='space-y-4'>
            ...
            <div className='relative'>
              ...
              <input type='text' value={address.mobile} className='pl-10 w-full border rounded-lg p-3 text-sm bg-gray-50' onChange={(e)=> setAddress((prev)=>({...prev,mobile:e.target.value}))}/>
            </div>

             <div className='relative'>
              <Home className='absolute left-3 top-3 text-green-600 'size={18}/>
              <input type='text' value={address.fullAddress} className='pl-10 w-full border rounded-lg p-3 text-sm bg-gray-50' onChange={(e)=> setAddress((prev)=>({...prev,fullAddress:e.target.value}))}/>
            </div>
            <div className='grid grid-cols-3 gap-3'>
              <div className='relative'>
                <Building className='absolute left-3 top-3 text-green-600 ' size={18}/>
                <input type='text' value={address.city} placeholder='city' onChange={(e)=> setAddress((prev)=>({...prev,city:e.target.value}))}className='pl-10 w-full border rounded-lg p-3 bg-gray-50'/>
              </div>
               <div className='relative'>
                <Navigation className='absolute left-3 top-3 text-green-600 ' size={18}/>
                <input type='text' value={address.state} placeholder='state' onChange={(e)=> setAddress((prev)=>({...prev,state:e.target.value}))} className='pl-10 w-full border rounded-lg p-3 bg-gray-50'/>
              </div>
               <div className='relative'>
                <Building className='absolute left-3 top-3 text-green-600 ' size={18}/>
                <input type='text' value={address.pincode} placeholder='pincode' onChange={(e)=> setAddress((prev)=>({...prev,pincode:e.target.value}))} className='pl-10 w-full border rounded-lg p-3 bg-gray-50'/>
              </div>
            </div>
            <div className='flex gap-2 mt-3'>
              ...
              <button className='bg-green-600 text-white px-5 rounded-lg hover:bg-green-700 transition-all font-medium' onClick={handleSearchQuery}>{loading?<Loader2 size={16} className='animate-spin'/>:"Search"}</button>
            </div>
      ...
      onClick={handleCurrentLoacaiton}
      >
      <LocateFixed size={22}/>
      </motion.button>
        ...
        >
         ...
         <motion.button
         whileTap={{scale:0.93}} className='w-full mt-6 bg-green-600 text-white py-3 rounded-full hover:bg-green-700 transition-all font-semibold' 
         onClick={()=>{
          if(paymentMethod=="cod"){
            handleCod()
          }else{
          handleOnelinePayment ()
          }
         }}
         >
