...
import { IMessage } from '@/model/message.models'
...
function DeliveryChat({ orderId, deliveryBoyId }: props) {
  ...
  return (
    ...
    <div className='flex justify-between items-center mb-3'>
      ...
      onClick={()=>getSuggestion()}><Sparkle size={14}/>{loading?<Loader className='w-5 h-5 animate-spin'/>:"AI suggest"}</motion.button>
     ...
     <div className='flex gap-2 flex-wrap mb-3'>
        ...
        <AnimatePresence>
          {messages?.map((msg, index) => (
            ...
            ${msg.senderId.toString() === deliveryBoyId
                ...
                <p>{msg.text}</p>
                <p className="text-[11px] opacity-70 text-right mt-1">
      ...
      <div className='flex gap- 2 mt-3 border-t pt-3'>
        ...
        <button className='bg-green-600 hover:bg-green-700 -3 rounded-xl text-white' onClick={() => sendMsg()}><Send size={20} /></button>
      </div>
