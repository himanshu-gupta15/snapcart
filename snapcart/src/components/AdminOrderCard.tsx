...
function AdminOrderCard({order}:{order:IOrder}) {
  ...
  // 1. Add a useEffect to sync internal state with prop changes
useEffect(() => {
  if (order.status) {
...
// 2. Add a fallback in your JSX to prevent "delivered" logic from taking over 
// if status is briefly undefined
  ...
  status === "delivered"
    ...
    ? "bg-blue-100 text-blue-700"
    : "bg-yellow-100 text-yellow-700" // This handles "pending" AND undefined
}`}>
  {status || "pending"}
</span>
