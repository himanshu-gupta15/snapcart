 ...
 const handleSearch=(e:FormEvent)=>{
  ...
  const sideBar=menuOpen?createPortal(
    ...
    transition={{type:"spring",stiffness:100,damping:14}}
     className='fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-9990
      bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white'
      >
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      {sideBar} 
      <Link href={"/"} className='text-white font-extrabold text-2xl sm:text-3xl tracking-wide hover:scale-105 transition-transform'>Snapcart</Link>
      {user.role==="user" && 
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'>
        ...
        {user.role==='admin' && <>
         ...
         <div className='md-hidden bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md ' onClick={()=>setMenuOpen(prev=>!prev)}>
          <Menu className='text-green w-6 h-6'/>
         </div>
        </>}
       
        <div className='relative' ref={profileDropDown}>
        <div className='bg-white rounded-full -11 h-11 flex items-center justify-center overflow-hidden shadow-md hover:scale-105 transition-transform ' onClick={()=>setOpen(prev=>!prev)}>
        ...
        <AnimatePresence>
          ...
          >
            ...
            {user.role=="user" &&<Link href={"/user/my-orders"} className='flex items-center gap-2 px-3 py-3 hover:bg-green-50 rounded-lg  font-medium' onClick={()=>setOpen(false)}>
        ...
        <AnimatePresence>
          ...
          >
            <Search className='text-gray-500 w-5 h-5 mt-2'/>
