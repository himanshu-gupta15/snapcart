...
function CategorySlider() {
  const categories=[
     ...
     { id:3,name:"Rice, Atta & Grains", icon:Wheat,color:"bg-orange-100"},
      {id:4,name:"Snack & Biscuits", icon:Cookie,color:"bg-pink-100"},
  ...
  return (
    ...
    >
      <h2 className='text-2xl md:text-3xl font-bold text-green-700 mb-6 text-center'>Shop by Cateogory</h2>
      {showLeft &&<button className='absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all ' onClick={()=>scroll("left")}><ChevronLeft/></button> }
      
      <div
      ref={scrollRef} className='flex gap-6 overflow-x-auto px-10 pb-4 scrollbar-hide scroll-smooth'>
        {categories.map((cat)=>{
          const Icon=cat.icon 
          return <motion.div
          key={cat.id}
      ...
      {showRight &&<button className='absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-green-200 rounded-full w-10 h-10 flex items-center justify-center transition-all' onClick={()=>scroll("right")}><ChevronRight className='w-6 h-6 text-green-700'/></button>}
