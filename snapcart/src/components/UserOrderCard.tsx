...
function UserOrderCard({order}:{order:IOrder}) {
  ...
  const getStatusColor=(status:string)=>{
    switch(status){
      ...
      case "out of delivery":
        return "bg-blue-100 text-blue-700 border-green-300"  
      case "delivered":
  ...
  return (
    ...
    >
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 linear-to-r from-green-50 to-white'>
        <div>
        ...
        <div className='flex flex-wrap items-center gap-2'>
         {status!=="delivered" && <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${order.isPaid
          ...
          <span className={`px-3 text-xs font-semibold border rounded-full ${getStatusColor(
            status
          )}`}>
          {status}
          </span>
        ...
        {order.paymentMethod=="cod"?<div className='flex items-center gap-2 text-gray-700 text-sm'>
          ...
          Online Payment
           ...
           {order.assignedDeliveryBoy && <> <div className='mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center  justify-between'>
                   <div className='flex items-centergap-3 text-sm text-gray-700'>
                     ...
                     <a href={`tel:${order.assignedDeliveryBoy.mobile}`} className='bg-blue-600 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-blue-700 transition'>Call</a>
                       ...
                       <button className='w-full items-center justify-center gap-2 bg-green-600 text-white font-semibold px-4 py-2 rounded-xl shadow hover:bg-green-700 transition' onClick={()=>router.push(`/user/track-order/${order._id?.toString()}`)}><Truck size={18}/>Track Your Order</button>
                        </>
                     } 
          ...
          <div className='border-t border-gray-200 pt-3'>
            ...
            <div className='mt-3 space-y-3'>
              {order.items.map((item,index)=>(
                <div className='flex justify-between items-center bg-gray-50 rounded-xl px-3 py-2 hover:bg-gray-100 transition'>
