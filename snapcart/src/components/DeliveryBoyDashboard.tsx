...
function DeliveryBoyDashboard({earning}:{earning:number}) {
  ...
  const fetchAssignments=async()=>{
      ...
      const handleAccept=async(id:string)=>{
       ...
       try{const result=await axios.get(`/api/delivery/assignment/${id}/accept-assignment`)
      fetchCurrentOrder()
      }catch(error){
        console.log(error)
      }
      ...
      const sendOtp=async()=>{
        ...
        console.log(result.data)
         setShowOtpBox(true)   // <-- THIS WAS MISSING
    setOtpError("")
...
setUserLocation({ latitude: 0, longitude: 0 })
  ...
  socket.on("order-completed", ({ assignmentId }) => {
            ...
            <div className='max-w-3xl mx-auto'>
              ...
              <div className='mt-6 bg-white rounded-xl border shadow p-6'>
                {!activeOrder.order.deliveryOtpVerification && !showOtpBox &&(
                  // <button className='w-full py-4 bg-green-600 text-center text-white rounded-lg' onClick={sendOtp}>{verifyOtpLoading?<Loader size={16} className='animate-spin text-white text-center'/>:"Mark as Delivered"}</button>
...
>
  ...
  return (
    <div className='w-full min-h-screen bg-gray-50 p-4 mt-25'>
     <div className='max-w-3xl mx-auto '>
      <h2 className='text-2xl font-bold  mb-4'>Delivery Assigments</h2>
      {assignments.map((a,index) => (
  <div key={index} className='p-5 bg-white rounded-xl shadow mb-4 border'>
    <p><b>Order Id</b> #{a.order?._id?.slice(-6)}</p>
    <p>{a.order?.address?.fullAddress ?? "Address not available"}</p>
